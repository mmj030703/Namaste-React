...
const RestaurantInfoCard = ({ restaurantObj }) => {
    ...
    const [menuListCardData, setMenuListCardData] = useState(filteredRestaurantData?.data?.cards[5]?.groupedCard?.cardGroupMap?.REGULAR?.cards?.slice(1));

    console.log(filteredRestaurantData);

    const {name, areaName, avgRating, totalRatingsString, costForTwoMessage, cuisines, sla, feeDetails} = filteredRestaurantData?.data?.cards[2]?.card?.card?.info;
